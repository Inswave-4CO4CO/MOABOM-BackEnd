...
public class MyPageService {
    ...
    public PagedResultDTO<MyWatchDTO> getWatchContentList(String userId, List<String> ottList, String type, int page) {
        String ottListForSql = ottList.stream()
                .map(ott -> "'" + ott + "'")
                .collect(Collectors.joining(","));

        PagedResultDTO<MyWatchDTO> myContentList = watchRepository.getContentByUserAndOttList(userId, ottListForSql, type, page, 5);
    ...
    //리뷰 리스트 가져오기
    public PagedResultDTO<MyReviewDTO> getReviewList(String userId, int page) {
